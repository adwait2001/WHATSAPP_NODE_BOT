...
def made_one_line(df = dfi.df):             #### made list of products in subcat
    ...
    for i in subcat:
        ...
        for j in prod_for_spec(i,df):
            ss += str(inte)+".     "+i+" "+j+"   "+ str(productinfo[j])+" \n"
            inte += 1
        dic[str(num)] = ss
        num += 1
        ss = "code       product       rate(inRs)\n"
...
doc = {"subcat":subcat,'company':product,"product":product}

productinfo.update({ 'litre':1000 ,"kilo":1000,"kg":1000,"mililitre":1,"grams":1,"ml":1,'gms':1})

doc.update({'quantity':["a",'1','10','2', '3','4', '5', 'one', 'two' ,'three', 'four', 'five' ,'half','per'],
...
text = doc['company']+doc['offers']+ doc["order"]+doc["product"]+doc["price"]+doc["quantity"]+doc['enquiry']+doc['subcat']+doc['weight']+doc['instruction']+doc['greetings']
